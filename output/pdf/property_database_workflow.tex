% Property/database workflow, refreshed 19 September 2026.
...
\clearpage
\subsection{Production precision metadata and atomic writes}
Schema 1 directly includes the full-index and Coulomb precision columns in
\code{theory_properties}. Their role is independent of schema versioning.
...
No schema migration or automatic data backfill is implemented. Fresh-schema
DDL precedes the explicit data transaction and is not rolled back with it.
...
with their matching cutoffs. It opens a short transaction, locks the parent
theory before the shared property/realization rows with
\texttt{SELECT ... FOR UPDATE}, rechecks the current cutoffs, and merges
...
\clearpage
\subsection{Disconnected-sector properties and persisted factor IDs}
\label{sec:sector-storage}
The public \code{split_disconnected_sectors} partitions the gauge/matter graph
as described in Section~\ref{sec:disconnected}. The basic property calculator
returns a materialized tuple of tuples of input factor IDs:
\begin{lstlisting}[language=Python]
"disconnected_sectors": (("left", "middle"), ("right",), ())
\end{lstlisting}
The example has two interacting components and one combined free-hyper sector.
IDs are input keys, not Cartan types. Zero-multiplicity matter does not connect
components; all free hypers are assigned once to the final empty-factor sector.
A single simple input normally uses factor ID \code{gauge}.
The property is \code{None} for a non-SCFT candidate and does not require index
or Coulomb-spectrum calculation.

\begin{tabularx}{\textwidth}{@{}Yp{72mm}@{}}
\toprule Stored field & Value for the example\\\midrule
\code{disconnected_sector_count} & \code{3}\\
\code{disconnected_sectors_json} &
\code{[["left","middle"],["right"],[]]}\\
\code{properties_json} keys &
\code{disconnected_sector_count} and \code{disconnected_sectors}\\\bottomrule
\end{tabularx}
The dedicated columns are nullable \code{INT UNSIGNED} and \code{JSON}.
Count is the number of returned sectors, including the optional combined
free sector. NULL denotes unknown/unpopulated data.

Factor IDs belong to realizations. The stored description therefore uses the
first stored realization (smallest \code{lagrangian_realizations.id}) for that
theory. Other realizations and duplicate aliases preserve it; the sector IDs
are excluded from shared physical-property equality checks. This prevents
equivalent input names from making the theory-level sector record ambiguous.

A successful new import inserts the realization, then
\code{_store_disconnected_sectors} writes both dedicated columns and their
combined-JSON keys inside the import transaction. The helper only fills
missing metadata for that first realization, preserving existing values and
computed indices/cutoffs. Failures roll back with the import. Normal duplicate
imports do not serve as a bulk enrichment operation.

The earlier sector schema-upgrade/backfill implementation was removed when
the full layout became production schema 1. No initializer now scans saved
inputs to populate old rows, and the removed migration CLI must not be used.
\code{test_n2_theory_db_sectors.py} retains normal storage tests for connected,
disconnected and free sectors, plus first-realization and duplicate-name behavior.
...
\end{lstlisting}
Options include \code{--host}, \code{--port}, \code{--unix-socket},
\code{--connect-timeout}, \mbox{\code{--char-cache-database}}, \code{--lie-executable},
