...
\fancyhead[R]{\small\sffamily\color{muted}15 September 2026}
...
\vspace{11mm}
\begin{note}{Anomaly GUI and concurrent database imports}
Saved index cutoffs and CPU limits drive the working anomaly builder, with
editable gauge lists, parallel checks/imports, character-cache preparation and
timestamped logs. Fresh imports avoid missing-row gap-lock deadlocks.
Validation includes 268 backend tests and 32 GUI tests. Separate index
calculation and schema-7 precision rules remain in place.
\end{note}
\vfill
...
Index cache-file arguments: 15 September 2026.\\
...
Status & Anomaly GUI, parallel imports, logs and deadlock repair documented;
...
\clearpage
\section{Package \texttt{gui}: anomaly-building workflow}\label{sec:gui}
...
the current 268-test backend validation; cache benchmarks were not rerun here.
