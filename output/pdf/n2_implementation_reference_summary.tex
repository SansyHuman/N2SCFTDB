...
\fancyhead[R]{\small\sffamily\color{muted}16 September 2026}
...
\vspace{11mm}
\begin{note}{Index calculation, reusable GUI actions and database controls}
This revision documents the completed index tab, retained searches, parallel
calculation, shared logs and bounded database retries. It adds disconnected
sectors, stored-index reuse, degree-bounded FORM multiplication and the
password-confirmed deletion dialog with its editable UI file.
...
Status & Both GUI tabs and Settings deletion are implemented;
...
cache preparation and persistent logs. & \S\ref{sec:gui}\\
...
\clearpage
\section{Package \texttt{gui}: anomaly, index and Settings workflows}\label{sec:gui}
...
New session coverage includes \code{test_disconnected_index.py},
\code{test_form_degree_bound.py}, \code{test_gui_index_search.py},
\code{test_gui_index_tab.py}, \code{test_gui_index_calculator.py},
\code{test_gui_logging_utils.py}, \code{test_database_clear.py} and
\code{test_gui_database_clear.py}. All paths are relative to \code{test/}.
They cover partition/reuse, formal exponential equality, retained selections,
worker lifecycle, logs, authentication and transactional deletion.

\subsection{Validation snapshots}
The most recent recorded full run in this session passed \textbf{370 tests in
65.584 s, no skips}, on 16 September. A new stdin-worker deletion test passed
separately afterward; extracting the dialog UI then passed 38 focused tests.
Sections~\ref{sec:degree-bounded}, \ref{sec:disconnected} and
\ref{sec:delete-settings} state the exact scopes. This documentation refresh
checks source contracts, builds, PDF text and rendered pages; it does not rerun
the application suites or access the production database.
...
the later combined-suite results; cache benchmarks were not rerun here.
