...
\fancyhead[R]{\small\sffamily\color{muted}19 September 2026}
...
\vspace{11mm}
\begin{note}{Search, CSV export, TFORM and the production schema}
This edition adds theory search with SQL charge filters, CSV downloads,
FORM/TFORM thread controls and the recorded 32-worker measurements.
Index search now includes known lower cutoffs. Public sector properties and
their storage are documented in the complete production schema 1.
Implementation-time tests and benchmarks retain their dates and scopes.
\end{note}
\vfill
...
Property/database: 12 September. GUI/concurrency: 14 September.
Index sectors/cache-file arguments: 15 September. FORM and Settings deletion:
16 September. Search/export and production schema: 19 September 2026.\\
...
Status & Anomaly, index and search tabs, CSV export and Settings deletion are implemented;
basic properties and index calculation remain separate.
...
\code{gui} & Editable settings and gauge lists, parallel SCFT imports,
cache preparation, search/CSV export and persistent logs. & \S\ref{sec:gui}\\
...
The baseline recurrence starts with $zA_D$ and successively substitutes
\begin{equation}
...
\newcommand{\sessionheading}{\subsubsection}
\clearpage
\input{form_degree_bound.tex}
\input{tform_session_runtime.tex}
...
has \code{PRAGMA user_version=1}; it is unrelated to MySQL theory schema version 1.
...
For \code{CharacterDecompositionCache}, \code{cache_directory=} selects a directory
for the default filename and \code{database_path=} selects an explicit file;
they are mutually exclusive. Its builder CLI exposes \code{--cache-directory}
and \code{--cache-database}. Index calculation instead accepts the single file
argument \code{char_cache_database_path=} or CLI option \code{--char-cache-database}.
...
\renewcommand{\sessionheading}{\subsection}
\input{disconnected_index_sectors.tex}
...
exact central charges and disconnected-sector factor IDs. It never computes a full index, a Coulomb index or the
...
\code{disconnected_sectors} & Materialized tuple of factor-ID tuples; JSON arrays on serialization.\\\bottomrule
\end{tabularx}
For valid non-candidates, flavor metadata remains available; central charges
and conformal-manifold dimension are \code{None}; sectors are also \code{None}, with no marginal gauge
...
The direct \code{index.n2_theory_index.calculate_index} API computes a formal
gauge projection without imposing the SCFT-candidate gate. The following
\code{common} wrappers enforce that gate before calculating index properties.
...
\code{run_form(program, form_executable=..., timeout=..., form_threads=1, tform_executable="tform")} writes a FORM program
in an isolated temporary directory, invokes the executable with quiet output,
and returns the output text. One thread selects FORM; larger counts select
TFORM with \code{-wN} (Section~\ref{sec:tform-runtime}). It turns missing executables, timeouts, nonzero
...
calculator returns it explicitly, and MySQL schema 1 stores the two cutoffs
...
\clearpage
\subsection{MySQL persistence and schema version 1}\label{sec:db}
...
Production schema 1 retains \code{half_hyper_units} in the shared flavor table and JSON,
...
(current value \code{"1"}). Independent of theory rows.\\[3pt]
\code{theories} &
...
Coulomb-index and Coulomb-spectrum JSON, two cutoff columns, sector count/factor-ID JSON, and combined \code{properties_json}.
...
\code{gauge_representation_json}. The full/half split is stored per realization.\\[3pt]
...
\clearpage
\subsection{Canonical matter and production import behavior}
...
\subsubsection{Production schema 1}
The test period has ended. \code{SCHEMA_VERSION = 1} names the complete current
ten-table layout, including decimal charge indexes, separate index/spectrum
fields, both cutoffs and disconnected-sector metadata. It is not the early
test-period schema once numbered 1.

\code{initialize_database} executes the current
\code{CREATE TABLE IF NOT EXISTS} definitions and records version 1 when the
metadata row is absent. Existing version 1 is accepted; any other recorded
version is rejected. Initialization does not alter existing table definitions,
relabel an old database or enrich its rows. Pre-release migrations, their
dispatch loop, the sector backfill API and the migration CLI were removed
with their tests. There is no longer a universal update command.

An existing test database must not be assumed compatible from its version
number alone. No user database was relabelled or modified by this cleanup.
SQLite caches have their own independent schema version.
...
\clearpage
\section{Package \texttt{gui}: anomaly, index, search and Settings}\label{sec:gui}
\input{gui_anomaly_workflow.tex}
\input{gui_index_settings_workflow.tex}
\input{gui_search_export_workflow.tex}
...
\code{test_n2_theory_db_indices.py} & Deferred worker,
...
Session coverage includes \code{test_form_threads.py},
\code{test_gui_theory_search.py}, \code{test_gui_search_tab.py},
\code{test_gui_theory_download.py}, \code{test_n2_theory_db_sectors.py}
and the index search/calculator tests, alongside the earlier disconnected,
degree-bound, logging and deletion cases. All paths are relative to
\code{test/}. Migration/backfill tests have been removed; fresh initialization,
version rejection and normal sector/index storage remain covered.

\subsection{Validation snapshots}
The 19 September production-baseline run executed \textbf{414 tests in
32.958 s: 374 passed, 40 skipped}, with live MySQL fixtures disabled.
An isolated temporary MySQL server then verified fresh schema 1, all ten
tables/current columns and repeated initialization. Its 22 integration tests
finished in 1.501 s: \textbf{20 passed, 2 skipped}; the two skips required a
password-protected account. Sector storage, normal independent index upgrades,
unknown-cutoff preservation and schema validation were exercised. The server
was shut down and removed; the user's database was not accessed.

Earlier session stages separately recorded 340/38 pass/skip for TFORM,
358/38 for SQL search, and 372/38 for CSV export. The export and charge-filter
sections describe their additional isolated MySQL checks.
These are implementation-time results. This documentation refresh checks
source contracts, builds, PDF text and rendered pages; it does not rerun
application suites or the expensive benchmarks.

The 14 September deadlock-fix run passed \textbf{268 backend tests in 33.518 s,
with no skips}, using Sage/FORM/LiE and password-protected disposable MySQL
8.0.46. The GUI documentation-time rerun passed \textbf{32 tests in 0.869 s}.
Section~\ref{sec:gui} records the test-harness repairs and real parallel stress
checks. Neither run used the user's production database or personal vault.

The earlier two-stage database implementation passed 238 tests in 21.920 s on
12 September, including migrations, concurrent upgrades, rollback, partial
retries and real stored indices. That result is retained as historical evidence.

The historical 10 September run recorded 207 passes and two MySQL skips.
...
NetworkX dependency supplies gauge-sector connected components. All tests,
including GUI fixtures, are under \code{test/}; see \code{test/README.md} for
focused commands and the opt-in live database/vault checks. The
...
\subsubsection{Running on another machine}
The session's macOS and LAN MySQL discussion was a portability assessment,
not a tested deployment. A second machine needs Sage/PyQt6 and compatible
FORM/TFORM and LiE binaries, with its own executable/cache paths. Export and
index worker counts use that machine's configured CPU budget.

To use the original PC's MySQL from another LAN host, the client must use the
server's LAN address over TCP, not its local Unix socket. The server listener,
firewall and a database-scoped account must allow the intended client. No
server, firewall or account configuration was changed in this session.
