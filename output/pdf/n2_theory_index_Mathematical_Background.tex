...
\textbf{Index-package reference: 19 September 2026}
\end{center}
\vspace{3mm}
...
This revision adds FORM/TFORM execution controls, CPU allocation and the
dated 32-worker benchmarks; the public sector API is documented as well.
Earlier mathematical and cache material is retained. GUI/search/export and
production schema 1 belong to the companion implementation reference.
Preserved pages 2--4 contain the original setup and baseline recurrence;
Section~\ref{sec:degree-bounded} gives the current generated code.
...
\quad Current degree-bounded loop, proof and example & \pageref{sec:degree-bounded}\\
\quad TFORM controls and 32-worker measurements & \pageref{sec:tform-runtime}\\
...
\quad Disconnected sectors and stored-index reuse & \pageref{sec:disconnected}\\
...
\newcommand{\sessionheading}{\subsection}
...
\code{_build_form_program} & Implements Eqs.~(7), (17), (14), and (20), with
the degree-bounded loop in Section~\ref{sec:degree-bounded}.\\
\code{run_form} & Shared helper in \code{common/form_utils.py}; runs FORM/TFORM in an
...
\code{split_disconnected_sectors} & Public graph partition; returns factor tuples
and hyper lists, retaining one free sector.\\
\code{calculate_index} & Parses input, then delegates sector partition,
optional stored-index reuse and truncated multiplication to
\code{calculate_index_internal} (Section~\ref{sec:disconnected}).\\
...
Use \code{--char-cache-database} \code{/path/to/cache.db} to select the character
database file. The corresponding Python argument is
\code{char_cache_database_path=}. The FORM database
...
result = calculate_index(
    ...
    char_cache_database_path="/tmp/fresh-index/characters.db",
...
Earlier references were checked on 10 September; the added software references
were checked on 16 September 2026. The splitting heuristic,
