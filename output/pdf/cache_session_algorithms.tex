...
\cacheheading{Large cache files and WAL lifetime}
A several-gigabyte cache is not by itself a reason to replace SQLite
\cite{sqlite-use}. For this implementation, distinguish indexed-key lookup
from decoding one large JSON payload, scanning candidate keys for a dependency
plan, and waiting for concurrent writes. Those costs need representative
measurements; this session did not benchmark a several-gigabyte cache.

In WAL mode, \code{-wal} holds the write-ahead log and \code{-shm} coordinates
access. Other open connections or an unclean process exit can leave these
files after a program finishes. SQLite normally checkpoints and cleans up on
the last clean close; their presence alone is not evidence of corruption
\cite{sqlite-temp}. Never remove a live WAL manually: committed data may still
be there. A cache context manager closes its current thread's connection;
that does not close another process's connection. No new checkpoint or
worker-finalizer policy was implemented as part of this discussion.
...
sage -python -m index.n2_theory_index theory.json --order 18 \
  --char-cache-database /tmp/index-demo/char_decomposition_cache.db \
...
The corresponding index Python keywords are \code{char_cache_database_path=}
and \code{form_cache_database_path=}.
