...
\[
 S_h=\{a:\ R_{h,a}\ne\mathbf1\}.
\]
\code{split_disconnected_sectors} builds an undirected NetworkX graph with
...
\textbf{Public API.} \code{split_disconnected_sectors(factors, hypermultiplets)}
returns a list of pairs: a tuple of \code{GaugeFactorData} and a list of
\code{HyperData} or \code{ProductHyperData}. Input factor IDs and order are
retained. The basic property calculator materializes these IDs as a tuple of
tuples, not lazy iterators; it returns \code{None} for a non-SCFT candidate.
The free sector contributes an empty tuple, serialized as an empty JSON array.
Database ownership of those IDs is described in the implementation reference.
