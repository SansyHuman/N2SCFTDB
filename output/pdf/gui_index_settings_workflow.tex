...
\textbf{Search missing or lower-order indices}, \textbf{Select all}, and
...
MySQL settings and \code{iter_lagrangian_index_jobs(upgrade=True)}, without
schema initialization or writes. It passes the saved full-index order $N$
and exact Coulomb cutoff $D$. A job qualifies when a full index, Coulomb
index or spectrum is SQL NULL/JSON null, or when a recorded full/Coulomb
cutoff is strictly below its own requested cutoff. The two comparisons are
independent; a high Coulomb cutoff does not satisfy a low full-index order.
Equal/higher known cutoffs need no upgrade. Unknown precision alone does not
qualify; a missing other component can still select that theory.
...
inputs, needed-field metadata and source identity in the controller.
...
spawned processes. With $C$ selected theories, resolved CPU budget $P$
and TFORM count $T$, at most
$\min(C,\max(1,\lfloor P/T\rfloor))$ workers run, with at most twice that many
outstanding tasks. One task contains one theory; scheduling is dynamic.
The setting $-1$ resolves to all logical CPUs, with fallback one.
Section~\ref{sec:tform-runtime} explains engine selection and memory limits.
...
Inner LiE/character work uses one process; FORM receives the selected thread count.
...
\begin{enumerate}
\item Recheck each retained theory/realization using
\code{needed_lagrangian_index_fields} with the current saved $N,D$.
This targeted check can add newly needed components after settings change
or omit work already completed by another writer. It does not repeat the
database-wide search or expand the retained theory selection.
\item Compute missing or lower-order components outside write transactions.
A stored full index does not constrain the separate Coulomb cutoff.
The complete spectrum is filled only when absent.
\item Save each successful component independently with upgrades enabled
(\code{missing_only=False}). Locked comparisons preserve concurrent
equal/higher results and unknown precision; a failure in another component
does not erase successful commits.
...
\item \code{delete_all_theory_data} checks production schema version 1, starts a transaction,
