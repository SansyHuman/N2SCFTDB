"""Index-tab search, checkbox selection and a reusable in-memory job snapshot."""
...
class IndexTabController(QtCore.QObject):
    ...
    def __init__(self, window, store, *, project_root=None):
        ...
        window.selectAllIndexGroupsCheckBox.setEnabled(False)

    def set_available(self, available):
        """Let the main window prevent a search while another tab is busy."""
        self._available = available
        self.window.searchEmptyIndicesButton.setEnabled(available and self.process is None)
    ...
    def selected_jobs(self):
        """Jobs for checked groups, ready for a future calculation action."""
        listing = self.window.emptyIndexGaugeGroupsList
    ...
    def _sync_select_all(self, *_):
        ...
        control.setEnabled(listing.count() > 0 and self.process is None)
        del blocker
    ...
    def search(self):
        ...
        self.window.searchEmptyIndicesButton.setEnabled(False)
        self.window.emptyIndexGaugeGroupsList.setEnabled(False)
        self._sync_select_all()
        self.activeChanged.emit(True)
        self.log("Starting search with the saved database settings…")
        process.start(sage, ["-python", "-B", "-u", "-m", "gui.index_search"])
    ...
    def _read_output(self, final=False, *, stderr=False):
        ...
        for line in lines:
            ...
            try:
                ...
                self.log("Invalid search worker response; results will not be replaced.", "ERROR")

    def _error(self, error):
        if self.process is None:
            return
        if not self._cancelled:
            self.log(f"Search process: {self.process.errorString()}", "ERROR")
    ...
    def _finished(self, exit_code, exit_status):
        ...
        self.window.emptyIndexGaugeGroupsList.setEnabled(True)
        self._sync_select_all()
